...
class DeleteLeaveType(APIView):
    def delete(self, request, item_id):
        leave_list = list(leave_type_collection.find({'_id': item_id}))
        if len(leave_list) > 0:
            return Response({"message": "Leave Type is already assigned to user"}, status=204)
        else:
            updated_leavetype = leave_type_collection.update_one(
            ...
            if updated_leavetype.modified_count > 0:
