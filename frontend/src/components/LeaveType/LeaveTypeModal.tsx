...
const apiBaseURL = process.env.NEXT_PUBLIC_API_BASE_URL;
  interface LeaveTypeModalProps {
    ...
    IsisAction: string;
    onClose: () => void; 
    itemDetails: { _id: string; vleave_type: string; estatus: string } | null; 
  ...
  const LeaveTypeModal: React.FC<LeaveTypeModalProps> = ({ isOpen, onClose, itemDetails, IsisAction, fetchData, page, perPage }) => {
    ...
    function resetFormData() {
      setFormData({ _id: "", leavetype: "", status: "" });  
    }
    const handleChange = (
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      validateForm();
      if (!formData.leavetype || !formData.status) {
      ...
      try {
        await axios.post(`${apiBaseURL}/api/leave-type/`, payload, {
          headers: {
        ...
        .then(response => {
          ...
          setFormData({ _id: "", leavetype: "", status: "" }); // Clear the form data
    ...
    useEffect(() => {
      if (itemDetails) {
        setFormData({
          _id: itemDetails._id || '',
          leavetype: itemDetails.vleave_type || '',
    ...
    return (
      ...
      style={{
        ...
        <form onSubmit={handleSubmit} className='grid gap-y-6'>
          ...
          <div>
            <Label htmlFor="Leave Type" text="Leave Type" required/>
            <InputField
              type="text"
              name="leavetype" 
            ...
            />
              {errors.leavetype && <p className='err' style={styles.error}>{errors.leavetype}</p>}
          </div>
          <div>
